...
ifneq "$(REQUIRED_MAKE_VERSION)" "$(EARLIER_MAKE_VERSION)"
    ...
    $(warning ../make/universal.make:494: *** makefile bug: local variable FIND_FALSE from scope setsid (with value "! -prune") was referred to in scope setsid.  Stop.)
endif
...
TARGET_OS_SCRIPT = $(LIB_PATH)/target-os.rb
...
JDK_ROOT_SCRIPT = $(LIB_PATH)/find-jdk-root.rb
...
EXTRA_INCLUDE_PATH += src/native/Headers
...
NATIVE_SOURCE_PATTERN = $(CURDIR)/../src/native/$(OS)/*/*.$(EXTENSION)
...
# ----------------------------------------------------------------------------
CLASS_PATH += .generated/classes
CLASS_PATH += $(wildcard lib/jars/*.jar)
...
makeInstallerName.deb = $(MACHINE_PROJECT_NAME)_$(1)_$(TARGET_ARCHITECTURE).deb
...
makeInstallerName.rpm = $(MACHINE_PROJECT_NAME)-$(1)-2.$(patsubst amd64,x86_64,$(TARGET_ARCHITECTURE)).rpm
...
$(WIX_COMPILATION_DIRECTORY)/$(MACHINE_PROJECT_NAME).wxs: lib/installer.wxs
	$(COPY_RULE)
...
.generated/local-variables.make: per-directory.make universal.make
	mkdir -p $(@D) && \
